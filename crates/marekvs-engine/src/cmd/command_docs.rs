...
static TABLE: &[CommandDoc] = &[
    ...
    with_args(cmd("json.get", -2, CRO, 1, 1, 1, "Returns the serialized JSON value at one or more paths.", "1.3.0", "json"),
        &[A_KEY,
          Arg { token: Some("INDENT"), optional: true, ..arg("indent", "string") },
          Arg { token: Some("NEWLINE"), optional: true, ..arg("newline", "string") },
          Arg { token: Some("SPACE"), optional: true, ..arg("space", "string") },
          Arg { optional: true, multiple: true, ..arg("path", "string") }]),
    with_args(cmd("json.mget", -3, CROM, 1, -2, 1, "Returns the value at the path from one or more keys.", "1.3.0", "json"),
        &[Arg { multiple: true, ..A_KEY }, arg("path", "string")]),
    with_args(cmd("json.mset", -4, CWM, 1, -1, 3, "Sets values at paths in multiple keys (NOT atomic across keys in marekvs).", "1.3.0", "json"),
        &[Arg { multiple: true, args: &[A_KEY, arg("path", "string"), arg("value", "string")], ..arg("triplet", "block") }]),
    with_args(cmd("json.del", -2, CWF, 1, 1, 1, "Deletes the value at the path (whole doc when the path is the root).", "1.3.0", "json"),
        &[A_KEY, Arg { optional: true, ..arg("path", "string") }]),
    with_args(cmd("json.forget", -2, CWF, 1, 1, 1, "Alias of JSON.DEL.", "1.3.0", "json"),
        &[A_KEY, Arg { optional: true, ..arg("path", "string") }]),
    with_args(cmd("json.type", -2, CRO, 1, 1, 1, "Reports the JSON type at the path.", "1.3.0", "json"),
        &[A_KEY, Arg { optional: true, ..arg("path", "string") }]),
    with_args(cmd("json.numincrby", 4, CWF, 1, 1, 1, "Increments the number at the path (LWW under concurrent writers).", "1.3.0", "json"),
        &[A_KEY, arg("path", "string"), arg("value", "double")]),
    with_args(cmd("json.nummultby", 4, CWF, 1, 1, 1, "Multiplies the number at the path (LWW under concurrent writers).", "1.3.0", "json"),
        &[A_KEY, arg("path", "string"), arg("value", "double")]),
    with_args(cmd("json.strappend", -3, CWF, 1, 1, 1, "Appends a JSON string to the string at the path.", "1.3.0", "json"),
        &[A_KEY, Arg { optional: true, ..arg("path", "string") }, arg("value", "string")]),
    with_args(cmd("json.strlen", -2, CRO, 1, 1, 1, "Reports the length of the string at the path.", "1.3.0", "json"),
        &[A_KEY, Arg { optional: true, ..arg("path", "string") }]),
    with_args(cmd("json.arrappend", -4, CWF, 1, 1, 1, "Appends values to the array at the path (RGA sequence; concurrent appends all survive).", "1.3.0", "json"),
        &[A_KEY, arg("path", "string"), Arg { multiple: true, ..arg("value", "string") }]),
    with_args(cmd("json.arrindex", -4, CRO, 1, 1, 1, "Searches the array at the path for the first occurrence of a value.", "1.3.0", "json"),
        &[A_KEY, arg("path", "string"), arg("value", "string"),
          Arg { optional: true, ..arg("start", "integer") },
          Arg { optional: true, ..arg("stop", "integer") }]),
    with_args(cmd("json.arrinsert", -5, CWF, 1, 1, 1, "Inserts values into the array at the path before the index.", "1.3.0", "json"),
        &[A_KEY, arg("path", "string"), arg("index", "integer"),
          Arg { multiple: true, ..arg("value", "string") }]),
    with_args(cmd("json.arrlen", -2, CRO, 1, 1, 1, "Reports the length of the array at the path.", "1.3.0", "json"),
        &[A_KEY, Arg { optional: true, ..arg("path", "string") }]),
    with_args(cmd("json.arrpop", -2, CWF, 1, 1, 1, "Removes and returns the element at the index of the array at the path.", "1.3.0", "json"),
        &[A_KEY, Arg { optional: true, ..arg("path", "string") },
          Arg { optional: true, ..arg("index", "integer") }]),
    with_args(cmd("json.arrtrim", 5, CWF, 1, 1, 1, "Trims the array at the path to the inclusive index range.", "1.3.0", "json"),
        &[A_KEY, arg("path", "string"), arg("start", "integer"), arg("stop", "integer")]),
    with_args(cmd("json.objkeys", -2, CRO, 1, 1, 1, "Reports the keys of the object at the path (lexicographic order).", "1.3.0", "json"),
        &[A_KEY, Arg { optional: true, ..arg("path", "string") }]),
    with_args(cmd("json.objlen", -2, CRO, 1, 1, 1, "Reports the number of keys of the object at the path.", "1.3.0", "json"),
        &[A_KEY, Arg { optional: true, ..arg("path", "string") }]),
    with_args(cmd("json.toggle", 3, CWF, 1, 1, 1, "Toggles the boolean at the path.", "1.3.0", "json"),
        &[A_KEY, arg("path", "string")]),
    with_args(cmd("json.clear", -2, CWF, 1, 1, 1, "Empties containers and zeroes numbers at the path.", "1.3.0", "json"),
        &[A_KEY, Arg { optional: true, ..arg("path", "string") }]),
    with_args(cmd("json.merge", 4, CW, 1, 1, 1, "RFC 7386 merge-patches the value at the path (per-field CRDT deltas).", "1.3.0", "json"),
        &[A_KEY, arg("path", "string"), arg("value", "string")]),
    with_args(cmd("json.resp", -2, CRO, 1, 1, 1, "Returns the value at the path as a RESP structure.", "1.3.0", "json"),
        &[A_KEY, Arg { optional: true, ..arg("path", "string") }]),
    with_args(cmd("json.debug", -2, CRO, 2, 2, 1, "Debug container: MEMORY reports stored-record bytes; HELP lists subcommands.", "1.3.0", "json"),
        &[arg("subcommand", "string"), Arg { optional: true, ..A_KEY },
          Arg { optional: true, ..arg("path", "string") }]),
    ...
    with_args(cmd("proto.bindings", -1, CRO, 0, 0, 0, "Lists protobuf prefix bindings.", "1.3.0", "proto"),
        &[Arg { token: Some("MATCH"), optional: true, ..arg("pattern", "pattern") }]),
    with_args(cmd("proto.set", -3, CW, 1, 1, 1, "Stores a validated protobuf message under a key, decomposed into per-field CRDT records.", "1.3.0", "proto"),
    ...
    with_args(cmd("proto.get", 2, CF, 1, 1, 1, "Returns the raw protobuf message bytes stored under a key.", "1.3.0", "proto"),
        ARGS_KEY),
    with_args(cmd("proto.info", 2, CF, 1, 1, 1, "Returns schema, version, type, storage format (whole|fields) and size of a stored protobuf value.", "1.3.0", "proto"),
        ARGS_KEY),
    ...
    with_args(cmd("proto.setfield", -4, CW, 1, 1, 1, "Sets message fields by dot-path as per-field CRDT writes (concurrent edits to different fields merge).", "1.3.0", "proto"),
